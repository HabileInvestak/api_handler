...
def get_login_mode(request):
    try:
        if request.method == readProperty ('METHOD_TYPE'):
            url = ApiHomeDict.get(readProperty("LOGIN_MODE"))[0].url
            print url
            apiName = readProperty ("LOGIN_MODE")
            #authorization = request.META.get(readProperty('AUTHORIZATION'))
            #authorization=authorization.split("-")
            #public_key4_pem = b64_decode(authorization[1].replace("\n",""))
            #tomcat_count= b64_decode(authorization[2].replace("\n",""))
...
def get_position_book(request):
    ...
    try:
        if request.method == readProperty ('METHOD_TYPE'):
            url = ApiHomeDict.get(readProperty("POSITION_BOOK"))[0].url
            apiName = readProperty ("POSITION_BOOK")
            authorization = request.META.get(readProperty('AUTHORIZATION'))
            authorization = authorization.split("-")
            public_key4_pem = b64_decode(authorization[1].replace("\n", ""))
            tomcat_count = b64_decode(authorization[2].replace("\n", ""))
...
def get_check_transaction_password(request):
    ...
    try:
        if request.method == readProperty ('METHOD_TYPE'):
            url = ApiHomeDict.get(readProperty("TRANSACTION_PASSWORD"))[0].url
            apiName = readProperty ("TRANSACTION_PASSWORD")
            authorization = request.META.get(readProperty('AUTHORIZATION'))
            authorization = authorization.split("-")
            public_key4_pem = b64_decode(authorization[1].replace("\n", ""))
            tomcat_count = b64_decode(authorization[2].replace("\n", ""))
...
def dataTypeValidation(dataType,Paramvalue,param,dict,validValues):
    ...
    if (isBlank(Paramvalue)):
        pass
    elif (dataType == readProperty('STRING')):
        pass
